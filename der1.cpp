...
void inorder(tree* tr) // симметричный обход (левый потомок, корень, правый потомок)
{
	if (tr)
	{
	...
	else if (!v->left || !v->right) //если только один ребенок
	{
		...
		else
		{
			if (!v->left) //если есть правый ребенок
			{
				...
				else p->right = v->right; ////ребенок удаляемого узла становится правым ребенком своего "деда"
...
void del_2(tree*& tr) // функция удаления нечетных
{
	if (tr)
	{
		tree* p = tr;
		del_2(tr->left);
		del_2(tr->right);
		if (p->inf % 2 != 0 && p->inf) Delete(tr, p); //9 4 8 7 6 3 5 2 1 0
	}
}
